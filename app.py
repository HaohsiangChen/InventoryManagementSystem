...
# Config MySQL
app.config['MYSQL_HOST'] = '10.120.28.5'
...
def products():
    ...
    #Get products
    result = cur.execute("SELECT * FROM prediction")
...
class ProductForm(Form):
    product_id = StringField('Product ID', [validators.Length(min=1, max=20)])
    product_name = StringField('Product Name', [validators.Length(min=1, max=20)])
    product_stock = StringField('Product Stock', [validators.Length(min=1, max=20)])
...
def add_product():
    ...
    if request.method == 'POST'and form.validate():
        ...
        #execute
        cur.execute("INSERT into prediction(food_ID, food_name, food_stock) VALUES(%s, %s, %s)",(product_id, product_name, product_stock,))
...
def edit_product(id):
    ...
    #Get article by id
    result = cur.execute("SELECT * FROM prediction where food_ID = %s", [id])
    ...
    form.product_id.data = str(product['food_ID'])
    form.product_name.data = product['food_name']
    form.product_stock.data = str(product['food_stock'])
    ...
    if request.method == 'POST'and form.validate():
        ...
        #execute
        cur.execute("UPDATE prediction SET food_ID=%s, food_name=%s, food_stock=%s WHERE food_ID=%s",(product_id, product_name, product_stock, id))
        #cur.execute("UPDATE prediction SET food_ID=%s WHERE food_ID=%s",(product_id, id))
...
def delete_product(id):
    ...
    #execute
    cur.execute("DELETE FROM prediction WHERE food_ID=%s", [id])
